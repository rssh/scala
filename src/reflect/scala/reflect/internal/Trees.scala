...
trait Trees extends api.Trees { self: SymbolTable =>
  ...
  /**
   * extractor used for annotated import, which accept imports 
   * and terms like Annotated(annotation,Import(..))
   */
  object MaybeAnnotatedImport
  {
    def unapply(tree:Tree): Option[(Import,List[Tree])] =
      tree match {
        case x@Import(tree,selectors) => Some((x,Nil))
        case Annotated(annotation,MaybeAnnotatedImport(x, rest)) => 
                                       Some((x,annotation::rest))
        case _ => None
      }
  }
