...
//  def Literal(x: Any) = new Literal(Constant(x))
  ...
  class LazyTreeCopier extends TreeCopierOps {
    ...
    def Export(tree: Tree, expr: Tree, selectors: List[ImportSelector]) = tree match {
      case t @ Export(expr0, selectors0)
             if (expr0 == expr) && (selectors0 == selectors) => t
      case _ => treeCopy.Export(tree, expr, selectors)
    }
  ...
  override protected def itransform(transformer: Transformer, tree: Tree): Tree = {
    ...
    tree match {
      ...
      case Export(expr, selectors) =>
        treeCopy.Export(tree, transform(expr), selectors)
      case Template(parents, self, body) =>
